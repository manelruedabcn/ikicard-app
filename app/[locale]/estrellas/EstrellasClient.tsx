...
import {
  getEstrellas,
  getEstrellasStatements,
  getEstrellasIntro,
  getEstrellasClosing,
  computeDominant,
  type EstrellaStatement,
} from '@/lib/estrellas-content'
...
export default function EstrellasClient({ userId, locale, volver, initial }: Props) {
  const supabase = createClient()
  const tn = useTranslations('nav')
  const statements = getEstrellasStatements(locale)
  ...
  async function finish(finalScores: Record<string, number>) {
    ...
    setPhase('result')
    window.scrollTo({ top: 0 })
    trackEvent('estrella_result', { tool: 'estrellas', dominant: dom ?? '' })
  ...
  return (
    <Shell locale={locale} tn={tn}>
      <div className="w-full max-w-md">
        {phase === 'intro' && <Intro locale={locale} onStart={() => setPhase('test')} />}
        {phase === 'test' && (
          <Test
            locale={locale}
