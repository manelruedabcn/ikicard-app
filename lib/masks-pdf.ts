...
// nada en el render inicial ni romper el SSR. Misma mecánica que PASO, con las
// clases propias de esta herramienta.

// Prepara el clon del informe para el PDF: enseña el pie con QR (que en pantalla
// está oculto) y esconde lo interactivo (botones y ejercicio), que no pinta.
function prepararClon(doc: Document) {
  doc.querySelectorAll<HTMLElement>('.mask-print-only').forEach(el => {
    el.style.display = 'block'
  })
  doc.querySelectorAll<HTMLElement>('.mask-no-export').forEach(el => {
    el.style.display = 'none'
  })
}

export async function generarMascarasPdf(el: HTMLElement, fileName = 'Mascaras.pdf', mode: 'download' | 'share' = 'download') {
  ...
  if (mode === 'share' && nav.share && nav.canShare && nav.canShare({ files: [file] })) {
    try {
      await nav.share({ title: 'Mi informe de Máscaras · IKIGAIER', text: 'Este es mi informe personal.', files: [file] })
      return
    } catch (e) {
